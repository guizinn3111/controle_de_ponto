  ...
  for (j = 1; j <= ultimoDia; j++) {
    ...
    var r       = mapaDias[dataStr]; // r é o registro para este dia, se existir

    // Se for sábado ou domingo, exibe "Folga" e pula para o próximo dia
    if (dt.getDay() === 0 || dt.getDay() === 6) {
      rows +=
        '<tr class="tr-folga">' +
          ...
          '<td></td>' + // Coluna para botões, vazia para folga
        '</tr>';
      continue; // Pula para o próximo dia
    }

    // --- Lógica para dias úteis (com ou sem registro) ---
    var entrada, saidaAlm, voltaAlm, saida, horas, jornMin, saldo, heHtml, abonoHtml, btnEditar, btnHE;

    if (!r) { // Se NÃO há registro para este dia útil
      // Valores padrão para um dia sem registro
      entrada = '';
      saidaAlm = '';
      voltaAlm = '';
      saida = '';
      horas = null; // Nenhuma hora trabalhada
      jornMin = getJornadaMinutos(colab, dataStr); // Pega a jornada para contexto de abono
      saldo = -jornMin; // Assume falta total para o saldo
      heHtml = '<span class="td-dash">—</span>'; // Nenhuma hora extra

      // Para dias sem registro, exibe "Falta" e o botão "Abonar"
...
if (abono) {
  abonoHtml =
    ...
    '<button class="btn-rm-abono" onclick="removerAbono(' + colab.id + ',\'' + dataStr + '\')">✕</button>';
} else {
  abonoHtml =
    '<span class="badge-late">Falta</span> ' +
    '<button class="btn-abonar" onclick="abrirModalAbono(' + colab.id + ',\'' + dataStr + '\',\'falta\')">Abonar</button>';
}
      // Botões de Editar e HE ficam vazios para dias sem registro, conforme pedido "nada além disso"
      btnEditar = '';
      btnHE = '';

    } else { // Se HÁ registro (r não é nulo)
      // Lógica existente para dias com registro
      entrada  = r.batidas[0] || '';
      ...
      if (horas) totalMin += horas; // Só adiciona ao total se houver horas reais
    ...
    // Esta parte monta a linha para TODOS os dias úteis, usando as variáveis definidas acima
